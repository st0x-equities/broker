...
bytes constant TRANSFERS = 
    ...
    "fee: decmial18-mul(output-size 2000000000000000)," // calculates the brokers fee
    "trade-output-post-fee: decmial18-mul(output-size 998000000000000000),"// calculates the trade amount without the fee
    "transfererc1155slist: sentinel," "transfererc721slist: sentinel," "transfererc20slist: sentinel,"
    "_ _ _ _: output-token broker caller trade-output-post-fee," // transfer trade amount of usdt after fees are taken from broker to caller
    "_ _ _ _: output-token broker ST0x fee," // transfer fee from broker to ST0x
    "burnslist: sentinel," // burn sentinels
     "_ _: caller 0 output-size," // mint flow20 to caller
    "mintslist: sentinel,"; //  mint sentinels
